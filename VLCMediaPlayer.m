...
- (void)mediaPlayerRecordingFinished:(NSString *)fileName;
...
- (instancetype)initWithDrawable:(id)aDrawable options:(NSArray *)options
{
    if (self = [super init]) {
        ...
        _position = 0.0f;
        _cachedState = VLCMediaPlayerStateStopped;
        _recordedFile = [[NSString alloc] init];
...
- (void)registerObservers
{
    ...
    libvlc_event_attach(p_em, libvlc_MediaPlayerRecordingFinished,HandleMediaRecordingFinished,    (__bridge void *)(self));
...
- (void)unregisterObservers
{
    ...
    libvlc_event_detach(p_em, libvlc_MediaPlayerRecordingFinished,HandleMediaRecordingFinished,    (__bridge void *)(self));
...
static void HandleMediaRecordingFinished(const libvlc_event_t * event, void * self)
{
    @autoreleasepool {
        [[VLCEventManager sharedManager] callOnMainThreadObject:(__bridge id)self
                                                     withMethod:@selector(mediaPlayerRecordingFinished:)
                                           withArgumentAsObject:@(event->u.media_player_recording_finished.psz_filename)];
...
- (BOOL)recordStart:(NSString *)recordingFileame
{
    if (libvlc_media_player_record_start(_playerInstance, [recordingFileame UTF8String]) == 0) {
        return TRUE;
    } else
        return FALSE;
}
...
- (BOOL)recordStop
{
    if (libvlc_media_player_record_stop(_playerInstance) == 0) {
        return TRUE;
    } else
        return FALSE;
}
...
-(void)mediaPlayerRecordingFinished:(NSString *)fileName
{
    _recordedFile = [fileName copy];
    NSLog(@"Recorded filename is:%@", fileName);
}
